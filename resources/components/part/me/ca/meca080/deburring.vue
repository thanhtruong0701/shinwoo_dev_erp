...
export default {
  ...
  methods: {
    ...
    async getLatestList() {
      const res = await this._getDataList("LG_SEL_MECA080_BY_GRP_NOCACHE", ["L","E"]);
      this.latestList = this._groupByArray(res, "LINE_NUM");
    },

    async getDayShiftList() {
      const res = await this._getDataList("LG_SEL_MECA080_BY_GRP_NOCACHE", ["D","E"]);
      this.dayShiftList = this._groupByArray(res, "LINE_NUM");
    },

    async getNightShiftList() {
      const res = await this._getDataList("LG_SEL_MECA080_BY_GRP_NOCACHE", ["N","E"]);
