<template>
  <v-tabs vertical :color="currentTheme">
    ...
    <v-tab-item eager>
      <v-card outlined>
        <GwGridLayout no-gutters flat align="start" containerClass="px-1 py-0" :containerHeight="height">
...
export default {
  ...
  methods: {
    ...
    async getLatestList() {
      const res = await this._getDataList("LG_SEL_MECA080_BY_GRP_NOCACHE", ["L","A"]);
      this.latestList = this._groupByArray(res, "LINE_NUM");
    },

    async getDayShiftList() {
      const res = await this._getDataList("LG_SEL_MECA080_BY_GRP_NOCACHE", ["D","A"]);
      this.dayShiftList = this._groupByArray(res, "LINE_NUM");
    },

    async getNightShiftList() {
      const res = await this._getDataList("LG_SEL_MECA080_BY_GRP_NOCACHE", ["N","A"]);
