<template>
<v-dialog id="hr-1050170-dialog" max-width="1200" v-model="dialogIsShow">
        ...
        <v-card-text>
            <v-row no-gutters>
                <hr-basic-filter :version="2"
                    ...
                    :foreigner="foreigner" :all_org="true"
                ></hr-basic-filter>
...
export default {
    ...
    computed:
    {
      ...
      buttonCustom2(){
        return [
            ...
            { name: "clear", action: 'CLEAR', icon_type:"CLEAR_DATA", mdi: "mdi-eraser" }
        ]
        },
    },
    ...
    methods: {
        ...
        async onSearch(p_search) {
            let pa = p_search;
            pa.push(this.period_pk);
            
            let dso = {
                type: 'grid',
                selpro: "HR_1050130_MANAGER_LIST",
                para: [...pa]
            }
        ...
        onSelect(action){
            ...
            if(action == 'CLEAR') {
                this.$emit("callBack", {THR_EMP_PK: null } );
            } else {
