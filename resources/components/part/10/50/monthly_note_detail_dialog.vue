  ...
  <BaseDialog v-model="dialogIsShow" >
    <v-card light :width="1600" :height="900">
      <v-card-title class="headline primary-gradient white--text py-2 px-2">
          <span>{{ $t("monthly_note_detail", "common") }}</span>
          <v-spacer></v-spacer>
      ...
      <v-card-text>
        ...
        <base-wrapper :height="630" outlined>
            <v-container fluid class="px-1 py-1" v-resize="onResize" >
                <v-card outlined v-for="(item, idx) in  masterData" :key="`period${idx}`" width="100%" class="my-1"> 
                    <v-card-title class="subtitle-1 primary-gradient white--text py-1"   @click="onDisplayTab(idx)" style="cursor: pointer;">
                        ...
                        <span class="ml-5">
                            {{ $t('manager') }}: {{ item.MANAGER_NM }}    
                        </span>
                    </v-card-title>
                    <v-card-text class="py-1" :id="`card${idx}`">
                        <v-row no-gutters>
                            <v-col cols="12" lg="3" >
                                <BaseTextarea  outlined :label="$t('employee_note')" rows="10" 
                                    v-model="item.REMARK_EMP"
...
export default {
    ...
    data: () => ({
        ...
        dsoEmpInfo: {
            type: 'process',
            updpro: 'hr_sel_1050150_emp_mon_info',
        },

        dsoMaster: {
            type: 'grid',
            selpro: 'hr_sel_1050150_emp_mon_note_m',
        },

        dsoDetail: {
            type: 'grid',
            selpro: 'hr_sel_1050150_emp_mon_note_d',
        },
    }),
    ...
    methods: {
        ...
        loadData(){
            if(this.masterData?.length > 0) {
                ...
                for(let item of this.masterData) {
                    ...
                    let pk = item.PK;
                    let _thr_note_monthly_emp_pk = item.THR_NOTE_MONTHLY_EMP_PK
                    let detail = this.detailData.filter(q => q.THR_NOTE_MONTHLY_EMP_PK == _thr_note_monthly_emp_pk );
                    try{
        ...
        async onExport(){
            let reportName = "1050170_monthly_note_emp_detail.xlsx";
            let reportPath = "report/10/50/standard/1050170_monthly_note_emp_detail.xlsx";
            ...
            for( let _master of this.masterData )  {
                ...
                cell.value = `Manager: ${_master.MANAGER_NM}`;

                let cells = exceljs.getCells(this, currIdx ,1, 14);
                cells.forEach((c, idx) => {
                ...
                //header
                let headers = ["Employee Note", "", "", "", "", "", "Feedback Overall", "", "", "Seq", "Factor", "Mandatory", "Description", "Feedback"];
                let rowHeader = worksheet.getRow(currIdx);
                rowHeader.values = [...headers];
                let headerCells = exceljs.getCells(this, currIdx ,1, 14);
                ...
                worksheet.mergeCells(`G${currIdx}:I${currIdx}`);
                currIdx++;
                ...
                details.forEach( (q, idx) => {
                    ...
                    let values = [_master.REMARK_EMP, "", "", "", "", "", _master.REMARK_MNG, "", "", idx+1, q.FACTOR_NM, mandatory, q.DESCRIPTION, q.MANAGER_FEEDBACK]
                    let rowValues = worksheet.getRow(currIdx);;
                    rowValues.values = values;
                    let rowCells = exceljs.getCells(this, currIdx ,1, 14);
                    rowCells.forEach(c => {
                ...
                try { worksheet.unMergeCells(`G${startMerge}:I${endMerge}`); } catch {}
                //if (startMerge != endMerge ) {
                    worksheet.mergeCells(`A${startMerge}:F${endMerge}`);
                    worksheet.mergeCells(`G${startMerge}:I${endMerge}`);
               // } 
