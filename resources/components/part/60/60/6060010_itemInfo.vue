<template>
  <v-container fluid class="pa-0" v-resize="onResize">
    ...
    <v-dialog id="item-acc-ap-dialog" max-width="1000" v-model="dialogIsShow">
      <v-card>
        <v-card-title class="headline primary-gradient white--text py-2">
          <span>{{ $t('imp_item_to_excel') }}</span>
          <v-spacer></v-spacer>
              ...
              <v-row align="center" justify="space-between">
                <v-col cols="12">
                  <v-card outlined tile>
                    <v-container fluid>
                        <v-col cols="12">
                            <GwImportExcelFile :label="$t('imp_item_to_excel')" :impTempFile="'report/60/60/imp_6060010_invoice.xlsx'" :impProc="'AC_IMP_6060010_ITEM'" 
                                 :impStartRow="2" :impEndCol="8" :impAddParam="[this.master_data.PK]" @onAfterImport="onAfterImport"/>
                        </v-col>
                    </v-container>
                  </v-card>
                </v-col>
...
export default {
  ...
  computed: {
    ...
    master_data() {
        return this.$attrs.hasOwnProperty("master-data") ? this.$attrs["master-data"] : {};
    }
  },
  watch: {
    paras(val) {
      ...
      if (val.length > 0) {
        ...
        if(this.txtCrca_Pk == "")
        {
          this.txtTotal_Trans = null
          this.txtTotal_Book = null
        }
      }
  ...
  methods:
  {
    ...
    onAfterLoad() {
      ...
      for (var i = 0; i < this.$refs.grdItemInfo_N.getDataSource().length; i++) {
        if(this.$refs.grdItemInfo_N.getDataSource()[i].ATTRIBUTE5 == 4){
            total_ext_amt_dis = this.$refs.grdItemInfo_N.getDataSource()[i].U_PRICE == 0 ? null : null;
            total_tr_amt_dis = this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT == 0 ? null : null;
            total_bk_amt_dis = this.$refs.grdItemInfo_N.getDataSource()[i].NET_BK_AMT == 0 ? null : null;
            this.$refs.grdItemInfo_N.getDataSource()[i].EXT_PRICE    = total_ext_amt_dis;
            this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT   = total_tr_amt_dis;
            this.$refs.grdItemInfo_N.getDataSource()[i].ORDER_TRAMT  = total_tr_amt_dis;
            this.$refs.grdItemInfo_N.getDataSource()[i].ORDER_UPRICE = total_tr_amt_dis;
            this.$refs.grdItemInfo_N.getDataSource()[i].NET_BK_AMT   = total_bk_amt_dis;
            this.$refs.grdItemInfo_N.getDataSource()[i].VAT_RATE     = null;
        }
        else {
          total_ext_amt_dis = (this.$refs.grdItemInfo_N.getDataSource()[i].U_PRICE == null ? 0 : this.$refs.grdItemInfo_N.getDataSource()[i].U_PRICE) * (this.$refs.grdItemInfo_N.getDataSource()[i].QTY == null ? 0 : this.$refs.grdItemInfo_N.getDataSource()[i].QTY);
          total_tr_amt_dis = (this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT == null ? 0 : this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT);
          total_bk_amt_dis = ((this.$refs.grdItemInfo_N.getDataSource()[i].NET_BK_AMT == null ? 0 : this.$refs.grdItemInfo_N.getDataSource()[i].NET_BK_AMT));

          //total_tr_amt +=  Number(this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT == null ? 0 : this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT);
          //total_bk_amt +=  Number(this.$refs.grdItemInfo_N.getDataSource()[i].NET_BK_AMT == null ? 0 : this.$refs.grdItemInfo_N.getDataSource()[i].NET_BK_AMT);
          ...
          if (this.txtTrans_Ccy === 'VND') {
            ...
            //console.log("console.log(total_ext_amt_dis);  " + total_ext_amt_dis);
            //console.log("console.log(total_tr_amt_dis);  " + total_tr_amt_dis);
            //console.log("console.log(total_ext_amt_dis);  " +total_ext_amt_dis);

            this.$refs.grdItemInfo_N.getDataSource()[i].EXT_PRICE = Number(total_ext_amt_dis).toFixed(0);
            this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT = Number(total_tr_amt_dis).toFixed(0);
            this.$refs.grdItemInfo_N.getDataSource()[i].ORDER_TRAMT = Number(total_tr_amt_dis).toFixed(0);
            this.$refs.grdItemInfo_N.getDataSource()[i].ORDER_UPRICE = Number(total_tr_amt_dis).toFixed(0);
            ...
            this.$refs.grdItemInfo_N.getDataSource()[i].EXT_PRICE = Number(total_ext_amt_dis).toFixed(2);
            this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT = Number(total_tr_amt_dis).toFixed(2);
            this.$refs.grdItemInfo_N.getDataSource()[i].ORDER_TRAMT = Number(total_tr_amt_dis).toFixed(2);
            this.$refs.grdItemInfo_N.getDataSource()[i].ORDER_UPRICE = Number(total_tr_amt_dis).toFixed(2);

          }

          this.$refs.grdItemInfo_N.getDataSource()[i].NET_BK_AMT = Number(total_bk_amt_dis).toFixed(0);
        }
    ...
    onAddAcntMaster(item) {
      ...
      }else if (this.lstGetData === 'IMP') {
        if(!this.master_data.PK){
            this.showNotification('warning', this.$t('cannot_modify_this_slip'), '', 5000);
        }else {
    ...
    checkUpdatedItem(data, colName) {
      ...
      this.calculateCellExtPrice();
      if(colName === 'ATTRIBUTE5'){
        if(Number(data.data.ATTRIBUTE5) == 4){
          data.data.EXT_PRICE     = null;
          data.data.NET_TR_AMT    = null;
          data.data.NET_BK_AMT    = null;
          data.data.ORDER_TRAMT   = null;
          data.data.ORDER_UPRICE  = null;
          data.data.VAT_RATE      = null;
        }
      }
    },
    ...
    async calculateCellExtPrice() {
      if (this.txtCrca_Pk > 0) {
        ...
        else if (this.txtVat_Tax == '01' || this.txtVat_Tax == 'none') {
          v_vat_rate = 0;
        } else {
          v_vat_rate = Number(this.txtVat_Tax);
        }
        console.log("this.txtVat_Tax  " ,this.txtVat_Tax, " v_vat_rate  ", v_vat_rate)
        if(this.$refs.grdItemInfo_N != undefined)
        {
          for (var i = 0; i < this.$refs.grdItemInfo_N.getDataSource().length; i++) {
