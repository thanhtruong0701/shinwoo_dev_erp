          ...
          <v-row dense align="center" justify="center">
            <v-col cols="3">
              <v-card :height="gridTagsHeight" outlined class="overflow-x-overlay">
                <GwTreeView :items="items2" keyValue="id" keyDisplay="name" height="100%" selectionMode="multiple" checkBoxesMode="selectAll" itemsExpr="children" @onSelectionChanged="onSelectionChanged" />
              </v-card>
            </v-col>
            <v-col cols="9" class="py-0">
              <v-card outlined tile v-resize="onResize" :height="gridTagsHeight">
                ...
                <v-row dense>
                  <v-col cols="12">
                    <line-chart :key="key" ref="lineChart" :data="data" :legendDisplay="true" :legendPos="'right'" @chartResetZoom="resetZoom" @chartPointClick="onPointClick" @chartDisplayPoint="displayPoint" v-if="dialogIsShow"></line-chart>
                  </v-col>
                </v-row>
              </v-card>
            </v-col>
...
export default {
  ...
  computed: {
    gridHeight() {
      return this._calculateHeight(this.formContainerHeight, 25);
    },
    gridTagsHeight() {
      return this._calculateHeight(this.formContainerHeight, 55);
    },
    grdDetailHeader() {
  ...
  watch: {
    dialogIsShow(val) {
      if (val === true) {
        this.data_tags.forEach((item) => {
          this.uom_tags.forEach((item2) => {
            if (item.id === item2.id) {
              item["uom"] = item2.uom
            }
          })
  ...
  methods: {
    ...
    async getMachineTags() {
      ...
      let uomType = this.uom_tags.filter((x) => x.id === this.title)[0].uom;
      let tableType = this.data_tags[0].table_name;

      this.data_tags.forEach((item, index) => {
        if (item.id != "" && item.id != "N/A" && item.uom == uomType && item.table_name == tableType) {
          let listObj = {};
          listObj.id = index;
          listObj.name = item.id;
          listObj.field = item.name;
          listObj.table = item.table_name;
          listObj.selected = this.tag == item.name ? true : false;
          listObj.uom = item.uom;
    ...
    /* async getPosition() {
      const position_lst = await this._callProcedure(
        "mes_sel_meyn110_select_position"
      );
      this.position_list = position_lst;
      this.selected_position = position_lst[0].pn;
    }, */
    async getPagesCount() {
      if ((this.POP_SMU_EVENT_ROLL + this.POP_SMU_YBMSPARAM + this.POP_SMU_GPINFO + this.POP_SMU_EVENT_HEATER).includes(this.tag)) {} else {
    ...
    async onExport() {
      try {
        ...
        const report_path = "report/me/yn/rpt_meyn140.xlsx";        
        const report_name = "rpt_meyn140";
        this.$emit("onReport", canvasURL, this.strTags, report_path, report_name);
      } catch (error) {
