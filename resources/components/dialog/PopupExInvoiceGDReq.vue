  ...
  <v-dialog id="bank-dialog" max-width="1400" v-model="dialogIsShow" transition="dialog-top-transition" persistent>
    <v-card>
      <v-card-title class="headline primary-gradient white--text py-2">
        <span>{{ $t("ex_invoice_goods_delivery_request") }}</span>
        <v-spacer></v-spacer>
      ...
      <v-container fluid class="pa-2" v-resize="onResize">
        <v-row dense>
          ...
          <v-col md="4">
            ...
            <BaseInput :label="$t('partner_name')" v-model="modelSearch.PARTNER_NAME">
            <template v-slot:append>
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-icon v-on="on" :color="currentTheme" @click="(modelSearch.PARTNER_PK = '',modelSearch.PARTNER_NAME = '')">mdi-eraser</v-icon>
                </template>
              </v-tooltip>
            </template>
          </BaseInput>
        ...
        <v-row dense>
          <v-col md="12">
            <DataGridView column-resizing-mode="widget" ref="grdMaster" select_mode="Multiple" :max_height="heightGrdMaster" :header="headerGrdMaster" sel_procedure="LG_SEL_5010050_GDREQ_1" :filter_paras="[modelSearch.DATE_FROM,modelSearch.DATE_TO,modelSearch.REF_NO,modelSearch.PARTNER_NAME,modelSearch.PARTNER_PK]" upd_procedure="LG_SEL_5010050_GDREQ_1" :update_paras="['PK','ITEM_NAME']" />
          </v-col>
        </v-row>
        <v-row dense>
        ...
        <v-row dense>
          <v-col md="12">
            <DataGridView column-resizing-mode="widget" ref="grdDetail" select_mode="Multiple" :max_height="heightGrdMaster" :header="headerGrdDetail" sel_procedure="LG_SEL_5010050_GDREQ_2" :filter_paras="[]" upd_procedure="LG_SEL_5010050_GDREQ_2" :update_paras="['PK','ITEM_NAME']" @cellClick="onClickCell" @cellDblClick="onDblClickCell" />
          </v-col>
        </v-row>
      </v-container>
    </v-card>
