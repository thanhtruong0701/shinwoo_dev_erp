<template>
<div>
  <v-dialog id="upload-files-dialog" max-width="1600" v-model="dialogIsShow">
    <v-card>
      ...
      <v-card-text class="pa-0">
        <v-container>
          <v-row class="pt-2" align="center" justify="space-between" v-show="obj.ALLOW_UPLOAD">
          ...
          <v-row class="pt-2" align="center" justify="space-between">
            <v-col offset-md="11" md="1" class="pt-0">
              <v-row class="mr-1">
    ...
    dialogIsShow(val) {
      if (val === false) {
        ...
        try {
          this.fileSave = null;
          this.txtFile = [];
					this.P_MASTER_PK = this.obj.TABLE_PK
					this.title1 = 'view_files'
					if(this.obj.ALLOW_UPLOAD) this.title1 = 'upload_files'
          setTimeout(() => {
