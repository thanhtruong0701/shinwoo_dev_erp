...
<v-dialog id="bank-dialog" max-width="1400" v-model="dialogIsShow" transition="dialog-top-transition" persistent>
  <v-card>
    <v-card-title class="headline primary-gradient white--text py-2">
      <span>{{ $t("get_item_from_result") }}</span>
      <v-spacer></v-spacer>
    ...
    <v-container fluid class="pa-2" v-resize="onResize">
      <v-row dense>
        ...
        <v-col md="2">
          <BaseCheckbox :label="$t('all_cust')" true-value="Y" false-value="N" v-model="chkAllCust" />
        </v-col>
        <v-col md="2">
      ...
      <v-row dense>
        <DataGridView column-resizing-mode="widget" ref="grdSearch" :header="headerGrdSearch" :filterRow="true" select_mode="Multiple" :auto_load="false" :max_height="heightGrdSearch" sel_procedure="LG_SEL_5010011_3_NC" :filter_paras="[ objSend.TCO_BUSPARTNER_PK, txtFromDate, txtToDate, txtPOSONo, txtItem, 'DETAIL', chkAllCust ]" />
      </v-row>
      <v-row dense>
  ...
  methods: {
    ...
    initHeader() {
      this.headerGrdSearch = [
        ...
        { dataField: "REQ_BAL_QTY", caption: this.$t("req_bal_qty"), width: 120, allowEditing: false, formatFloat: 2, dataType: "number", }, 
        { dataField: "CURRENT_QTY", caption: this.$t("current_qty"), width: 120, allowEditing: false, formatFloat: 2, dataType: "number", }, 
        { dataField: "TLG_SA_SALEORDER_D_PK", caption: this.$t("tlg_sa_saleorder_d_pk"), width: 120, allowEditing: false, visible:false }, 
        { dataField: "PROD_DATE", caption: this.$t("prod_date"), width: 120, allowEditing: false, },
      ];
    },
  },
};
