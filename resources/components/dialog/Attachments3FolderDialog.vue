<template>
  <v-dialog
    id="attachments-to-folder-dialog"
    max-width="1000"
        ...
        <v-row no-gutters>
          <v-col md="12">
            <BaseTabs :height="48">
              <template #start>
                <div class="pr-3">
                  <span class="headline white--text py-2">{{
                    $t("attachments")
                  }}</span>
                </div>
              </template>
              <template #end>
                <v-icon dark @click="dialogIsShow = false"
                ...
                <v-row dense class="pt-1">
                  <v-col md="12" class="py-0">
                    <DataGridView
                      ...
                      sel_procedure="SYS_SEL_ATTACH_DIALOG_NOCACHE"
                      upd_procedure="SYS_DEL_ATTACHMENTS_DIALOG"
                      :filter_paras="filterPara1"
                      :update_paras="['PK', 'TABLE_NAME']"
...
export default {
  ...
  props: {
    ...
    proc: {
      type: String,
      default: "SYS_INS_ATTACHMENTS3F_DIALOG",
    },
    folder: {
      type: String,
      default: "File",
  ...
  methods: {
    ...
    async onAfterLoad() {
      this.imgList = await [];
      let imgAttachmentsList = await this.$refs.grdAttachments
        .getDataSource()
        .filter((i) => i.FILE_EXT == "png" || i.FILE_EXT == "jpg" || i.FILE_EXT == "jpeg");
      for (let i = 0; i < imgAttachmentsList.length; i++) {
        if (imgAttachmentsList[i].FILE_BASE_URL) {
          try {
            await this.$axios
              .$get(
                "/dso/getfile?file_name=" +
								imgAttachmentsList[i].FILE_BASE_URL +
                  "&type=1",
                { responseType: "blob" }
              )
              .then(async (res) => {
                if (res) {
                  await this.imgList.push(
                    window.URL.createObjectURL(
                      new Blob([res], { type: imgAttachmentsList[i].FILE_TYPE })
                    )
                  );
                }
              });
      ...
      if (!this.imgList.length) {
        await this.imgList.push(require("@/assets/images/no_image_available.png"));
      }
    },
    ...
    async changeFile(files) {
      for (let i = 0; i < files.length; i++) {
        ...
        const tempFile = file;
        const newFileName = tempFile.name.split(".")[0] +"-web-" +moment().format("YYYYMMDD_HHmmss") + '.' + tempFile.name.split('.').pop();
        const formData = new FormData();
        formData.append("file", tempFile, newFileName);
        formData.append("folder", this.folder);
        formData.append("location2", JSON.stringify(this.objSend));
        await this.$axios({method: "post", url: "/dso/uploadfile3folder", data: formData}).then(async (res) => {
          if (res.data) {
