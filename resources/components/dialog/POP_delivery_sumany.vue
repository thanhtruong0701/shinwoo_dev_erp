<template>
  <v-dialog id="delivery-dialog" max-width="1200" v-model="dialogIsShow">
    <v-card>
...
export default {
  ...
  computed: {
    ...
    filterParams() {
      const yyyymm = this.p_yearly && this.p_month ? `${this.p_yearly}${String(this.p_month).padStart(2, '0')}` : '';

      return [yyyymm, this.p_factory_pk || '', this.p_partner_pk || '', this.searchText || ''];
    }
  },
  ...
  watch: {
    dialogIsShow(val) {
      if (val === true) {
        console.log('Dialog opened with params:', {
          ...
          yyyymm: `${this.p_yearly}${String(this.p_month).padStart(2, '0')}`
        });
        this.searchText = '';
        this.$nextTick(() => {
          this.onSearch();
    ...
    searchText(val, oldVal) {
      // Skip if dialog just opened (initial empty value)
  ...
  methods: {
    ...
    async onSearch() {
      ...
      if (!this.p_yearly || !this.p_month) {
        this.$toast.warning(this.$t('Please select year and month'));
        return;
      }
