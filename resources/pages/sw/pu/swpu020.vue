        ...
        <v-row class="pt-1" dense>
            <v-col :md="cols_left" class="clsLeft">
                <v-card class="pa-1" outlined tile>
                    ...
                    <!-- <v-row class="pt-0" dense>
                        <v-col md="12">
                            <BaseSelect :label="$t('PO NO')" v-model="modelSearch.P_PO_DEPT_PK"
                                :lstData="dataList.lstDeptSearch" item-text="TEXT" item-value="PK" />
                        </v-col>
                    </v-row> -->
                    <v-row class="pt-0" dense>
                        <v-col md="12">
                    ...
                    <v-row class="pt-0" dense>
                        <v-col cols="12">
                            <DataGridView column-resizing-mode="widget" ref="grdSearch" sel_procedure="LG_SEL_SWPU020"
                        ...
                        <v-col md="8" style="text-align: right">
                            <div class="d-flex justify-end">
                                ...
                                <BaseButton icon_type="cancel" :btn_text="$t('cancel')"
                                    ...
                                    :disabled="isProcessing || modelMaster.STATUS != 'APPROVED'" />-->
                                <BaseButton icon_type="pdf" :btn_text="$t('pdf')" @onclick="On_Submit_Cancel('btnPrint')"
                                    :disabled="isProcessing" /> 
                    ...
                    <v-row class="pt-2">
                        <v-col cols="12" class="pt-0">
                            <DataGridView column-resizing-mode="widget" ref="grdDetail"
                                sel_procedure="LG_SEL_SWPU020_2_NOCACHE" upd_procedure="LG_UPD_3010050_2"
...
export default {
    ...
    methods: {
        ...
        async initHeade2() {
            ...
            // Extract summary type from headerDetail
        ...
        initHeader() {
            this.headerSearch = [
                ...
                { dataField: 'SUBMIT_YN', caption: this.$t('Submit YN'), allowEditing: false,width: 70 },
        ...
        async onSubmit(action) {
            ...
            if (row) {
                this.showNotification("success", "Information", this.$t("Submit Success"));
                this.onUpdMaster("select");
            }
        },
        ...
        async onReport(PK, report_type, type_rpt = 0) {
            ...
            let report_name = "";
            let excel = [];
            let proc1 = "LG_RPT_SWPU020_2"
            switch (type_rpt) {
                case 1:
                    proc1 = "LG_RPT_3010050_3"
                    break;
                default:
                    proc1 = "LG_RPT_SWPU020_2"
                    break;
            }
            ...
            switch (report_no) {
                case "01": {
                    report_path = "report/sw/pu/rpt_payment.xlsx";
                    ...
                    excel = [
                        //sheet1
                        {
                            sheet: 1,
                            insertRange: [
                                { range: "A1:T4", proc: "LG_RPT_COMPANY_INFO_02", params: [this.modelMaster.TCO_COMPANY_PK], imageColumns: ["COM_PIC"], },
                                { range: "A5:I19", proc: "LG_RPT_SWPU020_1", params: [...p_param], }, //header
                            ],
