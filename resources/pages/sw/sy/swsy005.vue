    ...
    <v-row dense>
      <v-col md="12">
        <v-row dense>
          <v-col cols="5">
            <v-row align="center" justify="space-between" no-gutters>
              <v-col md="4" class="pl-5">
                <BaseSelect :label="$t('payment_Type')" v-model="itemovertime" :lstData="lstdatetype" item-text="NAME"
          ...
          <v-col md="2" pl-2>
            <BaseSelect :label="$t('Report_Type')" v-model="itemreporttype" :lstData="lstreporttype" item-text="NAME"
              item-value="CODE" :text_all="$t('')" />
          </v-col>
        ...
        <v-row dense>
          <BaseGridView column-resizing-mode="widget" ref="grdViewTab3" :max_height="limitHeight"
            ...
            sel_procedure="SW_SEL_SWSY005_NOCACHE" upd_procedure="SW_UPD_SWSY005" :filter_paras="[
              this.itemovertime,
              this.date_from,
              this.base_date,
...
export default {
  ...
  data: () => ({
    ...
    lstreporttype: [
      {
        NAME: "Employee Register overtime",
        CODE: "01"
      },
      {
  ...
  methods: {
      ...
      for (let i = 0; i < arrDataSource.length; i++) {
        ...
        if (!this._hasValue(arrDataSource[i].REQUEST_DATE_FROM)) {
          this.$refs.alertDialog.showAlert(
            "warning",
            "warning",
            this.$t("pls_input_line_DATE_FROM_for_all_detail")
          );
          return false;
        }
        if (!this._hasValue(arrDataSource[i].REQUEST_HOUR)) {
          this.$refs.alertDialog.showAlert(
            "warning",
            "warning",
            this.$t("pls_input_line_HOUR_for_all_detail")
          );
          return false;
        }
        ...
        if (!this._hasValue(arrDataSource[i].REASON)) {
          this.$refs.alertDialog.showAlert(
            "warning",
            "warning",
            this.$t("pls_input_line_REASON_for_all_detail")
          );
          return false;
        }
      }
    ...
    async onReport2() {
      ...
      if (this.itemreporttype == "02") {
        report_templet = "rpt_swsy004_rigister_overtime_entry";
        report = "rpt_swsy005_rigister_overtime_entry_02";
      } else if (this.itemreporttype == "01") {
        report_templet = "rpt_swsy004_rigister_overtime_employe";
        report = "rpt_swsy005_rigister_holiday_entry01";
      }
    ...
    initHeader() {
      this.headerGrdViewTab3 = [
        ...
        },
        {
          ...
          caption: this.$t("from_date(overtime)"),
          allowEditing: true,
          // fixed: true,
          dataType: "date",
        ...
        },
        {
          ...
          caption: this.$t("to_date(overtime)"),
          allowEditing: true,
          // fixed: true,
          dataType: "date",
          format: this.curLang.DATE_FORMAT,
        ...
        {
          dataField: "REASON",
          caption: this.$t("Reason(overtime)"),
          allowEditing: true,
          width: 220,
        },
        {
