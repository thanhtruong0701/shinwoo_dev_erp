    ...
    <v-row dense>
      <v-col md="12">
        <v-row dense>
          ...
          <v-col cols="5" class="pt-3">
            <v-row align="center" justify="space-between" no-gutters>
              <v-col md="6" class="pr-2">
                <BaseInput v-model="txt_search_employee_name" :label="$t('task_name')" @keyPressEnter="onSearch()"
                  @change="onSearch" />
              </v-col>
              <v-col md="6" pl-2>
                <BaseSelect :label="$t('startus')" v-model="itemreporttype" :lstData="lstreporttype" item-text="NAME"
                  item-value="CODE" :text_all="$t('ALL')" />
              </v-col>
          ...
          <v-col md="1">
            <template v-if="false">
              <div class="d-flex justify-center" v-if="false">
        ...
        <v-row dense>
          ...
          <BaseGridView column-resizing-mode="widget" ref="grdViewTab3" :max_height="limitHeight"
            ...
            sel_procedure="SW_SEL_SWSY012_NOCACHE" upd_procedure="SW_UPD_SWSY011" :filter_paras="[
              this.date_from,
              this.base_date,
  ...
  methods: {
    ...
    OnValidate() {
      ...
      for (let i = 0; i < arrDataSource.length; i++) {
        if (!this._hasValue(arrDataSource[i].TASK_NAME)) {
          this.$refs.alertDialog.showAlert(
            "warning",
            "warning",
            this.$t("pls_double_click_to_select_task_name")
          );
          return false;
        }
