let _SW10070 = {
  print: async (that, report_path, report_name, orderPK) => {
    try {
      ...
      const dsoMaster = { type: "grid", selpro: "SW_RPT_SW10070_PKL_M", para: [ orderPK ] };
      ...
      if(resultMaster.length) {
        ...
        for (let i = 0; i < resultMaster.length; i++) {
          ...
          if(i === 0) {
            for (let j = 0; j < fieldsMaster.length-1; j++) {
            ...
            const dsoDetail = { type: "grid", selpro: "SW_RPT_SW10070_PKL_D", para: [ orderPK, elA.COLOR ] };
