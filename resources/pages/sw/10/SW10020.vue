        ...
        <v-row  no-gutters >
            <v-col cols="4" class="pt-2">
                <DataGridView
                    ...
                    sel_procedure="SZ_SEL_SW10020_2"
                    :filter_paras="[
                                    this.sel_Company, 
                                    this.txt_PO_NO,
                     ...
                     <v-row dense align="center" justify="space-between">
                        <DataGridView
                            ...
                            :header="[ {  dataField: 'SEQ',       caption: this.$t('seq') , allowEditing: true  },
									...
									{  dataField: 'SERIAL_START_FORMAT',       caption: this.$t('SERIAL_START_FORMAT') , allowEditing: false},
                        ...
                        for(let row of this.$refs.gridData_D.dataList)
                        {
                            if (row.EPC_YN=='N')
                            {
								...
								for ( i = row.CURRENT_QTY; i <= row.MAX_QTY; i++)  // 
                                {
                                    ...
                                    let len_i=i + "";
									let len_upc=0;
									let tmp_serial_start="";
									len_upc=row.UPC_A_CODE;
									tmp_serial_start=row.SERIAL_START_FORMAT;
                                    insertRow.PK = "";
                                    ...
                                    if (len_i.length < 11)
                                    {
                                        for(let j=0;j<7-len_i.length;j++)
                                        {
                                            tmp_serial += '0';
                                        }
                                    }
                                    //tmp_serial = '1160' +tmp_serial+ i;//BEFORE tmp_serial = '1' +tmp_serial+ i; 
									tmp_serial = row.FACTORY_ID+''+tmp_serial_start+''+tmp_serial+ i;//BEFORE tmp_serial = '1' +tmp_serial+ i; 
