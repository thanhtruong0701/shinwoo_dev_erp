                ...
                <v-container fluid class="pt-0">
                    <v-row dense class="pa-1">
                       ...
                       <v-col cols="1">
						...
						<v-col cols="1">
							<div class="d-flex justify-end">						
								<BaseButton icon_type="print" :btn_text="$t('print-BC')"  @onclick="onReport" />
							</div>	
						 </v-col>	
						<v-col cols="1">
							<div class="d-flex justify-end">						
								<BaseButton icon_type="print" :btn_text="$t('print-CSV')"  @onclick="onReport2" />
							</div>	
						 </v-col> 
							<!--v-tooltip bottom>
								<template v-slot:activator="{ on }">
...
export default {
    ...
    components: {
       ...
       // partnerDialog: () => import("@/components/dialog/PartnerDialog"),
        tabOrder: () => import("@/components/part/sw/nk/swnk040_Order"),
        tabMCL: () => import("@/components/part/sw/nk/swnk040_MCL"),
       // tabSeason: () => import("@/components/part/sw/10/SW10110_Season"),
        //tabTranslate: () => import("@/components/part/sw/10/SW10110_Translate"),
        tabExport: () => import("@/components/part/sw/nk/swnk040_Export"),
    ...
    watch: {
        selectedTab(val) {
            ...
            if (val == "tabOrder") {//ORDER
                    ...
                    this.tab_staus=1;
					if(this.lstFactory=='ENG-ANTA')
					{
						//alert(this.lstFactory);
						this.impEndCol = 81;
						this.imp_start_row=6;
						this.imp_template_file = 'report/sw/nk/rpt_swnk040_templet_order.xlsx';
						this.imp_store_prod = 'SW_IMP_SWNK040_ANTA_ORDER_01';
					} 
            ...
            if (val == "tabMCL") {//MCL
				...
				//this.impValidCol=8;
                this.imp_template_file = 'report/sw/10/RPT_IMP_MCL_ANTA.xlsx';
                this.imp_store_prod = 'SW_IMP_ANTA_SWNK040_MCL_NOCACHE';
            }
    ...
    methods: {
        ...
        async onChangeCompany() {
			...
			const dso = { type: "list", selpro: "sp_sel_list_commoncode", para: ["SWWMT04"] };
                        const result = await this._dsoCall(dso, "select", false); 
                        if (result) 
                        {
      ...
      this.LengthUpper = lstCommoncode[0];
		...
		//alert(this.tab_staus);
                ...
                if(this.tab_staus==1)
				{
					...
					if(this.lstFactory=='JUDAL-VL')
					{
						
						this.impEndCol = 24;
						this.imp_start_row=2;
						this.imp_template_file = 'report/sw/nk/rpt_swnk040_judal_templet_order.xlsx';
						this.imp_store_prod = 'SW_IMP_SWNK040_ANTA_JUDAL_VL_ORDER';
					}
					if(this.lstFactory=='LW')
					{
						//alert(this.lstFactory);
        ...
        search() {
			...
			else
			{
				...
				{
					alert("pls,input PO NO to search for export data!");
				}*/
			}
        },
        addNew() {
	...
	async onReport() { 
		...
		{
				const data_seach = [{  
								P_FILE_NAME_NEW: "rptswsoNK040_",
								p_company_pk :this.selectedCompany,
								p_PartnerCD_NM :this.txtPartnerCD_NM,
								p_AccCD_NM :this.txtAccCD_NM,
								p_AccountType :this.selectedAccountType,
								p_month :this.month.replace("-", ""),
								p_factory_pk :this.lstFactory,
								p_month_to :this.month_to.replace("-", "")
						
					}, ];
					var file_nm ="rptswsoNK040_barcode_detail_nike" + "_" + this.user.USER_ID + ".xlsx";//rptswso710_barcode_detail_nike
					var url_path = "/report/rptswso420/rptswsoNK040_01";//D:\Webproject\gsf20\app\Controllers\Http\report\SW\SO\rptswso420.js-->rptswsoNK030_01
					try {
		...
		else
		{
			const data_seach = [{  
					...
					var file_nm ="rpt_swnk040_barcode_detail_v02_vl" + "_" + this.user.USER_ID + ".xlsx";//rptswso710_barcode_detail_nike
					var url_path = "/report/rptswso420/rptswsoNK040_02_VL";//D:\Webproject\gsf20\app\Controllers\Http\report\SW\SO\rptswso420.js-->rptswsoNK030_01
					try {
						const result = await this.$axios.$get(url_path, {
							responseType: "blob",
							params: {
	...
	//============end--------------------------	
	async onReport2() {
		if(this.lstFactory=="ENG-ANTA") //begin english anta not QR
		{
				   if (!this._hasValue(this.lstFactory))
				   {
					...
					let report_main = "rpt_swnk040_csv_v01";
					let report = "rpt_swnk040_csv_PO_"+this.txtAccCD_NM+"_"+this.month.replace("-", "");
					  try{
						this._ExcelSaveAs(await this.$axios.$get('/dso/makereport', {responseType: "blob", params: {
						  ...
						  excel: JSON.stringify([{
							insertRange: [{
							  range: "A1:T10", proc: "SZ_RPT_SWNK040_CSV_NOCACHE", params: [...p_param],//SW_SEL_SW10110_export_custom
							}],
							insertRows: [{
								startRow: 2, proc: "SZ_RPT_SWNK040_CSV_NOCACHE", params: [...p_param], stringColumns: [ "PO_NO","BARCODE"], subStyle: { font : {   bold: true   } , fill: {
														type: 'pattern',
														pattern:'solid',
														fgColor : { argb: 'FFE599'},
														bgColor : { argb: 'FFE599'}
			...
			if (!this._hasValue(this.lstFactory))
				   {
					...
					let report_main = "rpt_swnk040_judal_vl_csv";
					let report = "rpt_swnk040_csv_style_color_"+this.txtAccCD_NM+"_"+this.month.replace("-", "");
					  try{
						this._ExcelSaveAs(await this.$axios.$get('/dso/makereport', {responseType: "blob", params: {
						  template: "report/sw/nk/" + report_main + ".xlsx",
						  excel: JSON.stringify([{
							insertRange: [{
							  range: "A1:T10", proc: "SZ_RPT_SWNK040_JUDAL_VL_CSV_NOCACHE", params: [...p_param],//SW_SEL_SW10110_export_custom
							}],
							insertRows: [{
								startRow: 2, proc: "SZ_RPT_SWNK040_JUDAL_VL_CSV_NOCACHE", params: [...p_param], stringColumns: [ "PO_NO","BARCODE"], subStyle: { font : {   bold: true   } , fill: {
														type: 'pattern',
														pattern:'solid',
														fgColor : { argb: 'FFE599'},
														bgColor : { argb: 'FFE599'}
