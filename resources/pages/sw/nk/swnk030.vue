...
export default {
    ...
    components: {
       ...
       // partnerDialog: () => import("@/components/dialog/PartnerDialog"),
        tabOrder: () => import("@/components/part/sw/nk/swnk030_Order"),
       ...
       // tabSeason: () => import("@/components/part/sw/10/SW10110_Season"),
        ...
        tabExport: () => import("@/components/part/sw/nk/swnk030_Export"),
        ...
        selectedTab(val) {
            ...
            if (val == "tabOrder") {//ORDER
					...
					if(this.lstFactory=='VE-ANTA')
					{
						...
						this.imp_template_file = 'report/sw/nk/rpt_swnk030_templet_order.xlsx';
						this.imp_store_prod = 'SW_IMP_SWNK030_ANTA_ORDER_01';
					} 
      ...
      this.LengthUpper = lstCommoncode[0];
		...
		//alert(this.tab_staus);
                ...
                if(this.tab_staus==1)
				{
					...
					if(this.lstFactory=='VE-ANTA' || this.lstFactory=='VE-ANTA-2')
					{
						//alert(this.lstFactory);
						this.impEndCol = 74;
						this.imp_start_row=2;
		...
		async OnDeleteMaster() {
			  let tab = 4;
				   ...
				   const dso = {
					...
					para: [this.selectedCompany, this.month.replace("-", ""), tab, this.lstFactory, this.txtAccCD_NM, this.selectedAccountType]
	...
	async onReport() { 
		...
		const data_seach = [{  
			...
			var file_nm ="rptswsoNK030_barcode_detail_nike" + "_" + this.user.USER_ID + ".xlsx";//rptswso710_barcode_detail_nike
			var url_path = "/report/rptswso420/rptswsoNK030_01";//D:\Webproject\gsf20_shinwoo\app\Controllers\Http\report\SW\SO\rptswso420.js-->rptswsoNK030_01
			try {
	...
	let report_main = "rpt_swnk030_csv_02";
	let report = "rpt_swnk030_csv_02_PO_"+this.txtAccCD_NM+"_"+this.month.replace("-", "");
      try{
        this._ExcelSaveAs(await this.$axios.$get('/dso/makereport', {responseType: "blob", params: {
          ...
          excel: JSON.stringify([{
            insertRange: [{
              range: "A1:T10", proc: "SZ_RPT_SWNK030_CSV_NOCACHE", params: [...p_param],//SW_SEL_SW10110_export_custom
            }],
            insertRows: [{
				startRow: 2, proc: "SZ_RPT_SWNK030_CSV_NOCACHE", params: [...p_param], 
