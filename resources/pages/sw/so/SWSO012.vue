            ...
            <v-col md="12" fluid class="pa-1">
                <v-card class="pa-1" outlined tile>
                    <v-row dense align="center">
                        <v-col cols="3">
                            <v-row no-gutters>
                                <v-col cols="4" class="pr-1">
                                    <BaseDatePicker start v-model="dt_from" :label="$t('from_date')" default>
                                    </BaseDatePicker>
                                </v-col>
                                <v-col cols="4" class="pr-1">
                                    <BaseDatePicker start v-model="dt_to" :label="$t('date_to')" default>
                                    </BaseDatePicker>
                                </v-col>
                                <v-col cols="4">
                                    <BaseSelect :label="$t('Item Type')" v-model="selecteditemType"
                                        :lstData="lstItemType" item-text="text" item-value="value" />
                                </v-col>
                            </v-row>
                        </v-col>
                        <v-col cols="1">
                        ...
                        <v-col cols="1">
                            <BaseSelect :label="$t('startus')" v-model="itemreporttype" :lstData="lstreporttype"
                                item-text="NAME" item-value="CODE" @change="onStatusChange" />
                        </v-col>
                        ...
                        <v-col cols="1">
                            <BaseSelect :label="$t('Set Status')" v-model="selectedStatus" :lstData="statusOptions"
                                item-text="NAME" item-value="CODE" />
                        </v-col>
                        <v-col cols="1">
                            <BaseButton btn_type="icon" icon_type="set" :btn_text="$t('set-status')"
                        ...
                        <v-col cols="2">
                            <v-row no-gutters align="center">
                                <v-col cols="9" class="pr-1">
                                    <BaseDatePicker v-model="selectedConfirmDate" :label="$t('Set Confirm Date')"
                                        default />
                                </v-col>
                                <v-col cols="3">
                                    <BaseButton btn_type="icon" icon_type="set" :btn_text="$t('set-date')"
                                        :disabled="isProcessing"
                    ...
                    <v-row dense align="center" class="gap-4"> <!-- Thêm gap -->
                        <!-- <v-col md="2" class="pt-0">
                            <v-row justify="end" class="mr-1">
                                <BaseButton icon_type="excel" :btn_text="$t('excel_ICV')" :disabled="isProcessing"
                                    @onclick="onReport" />
                            </v-row>
                        </v-col> -->
                        <!-- <v-col md="1" class="pt-0">
                            <v-row justify="end" class="mr-1">
                                <BaseButton icon_type="excel" :btn_text="$t('excel_Backing_list')"
                                    :disabled="isProcessing" @onclick="onReport2" />
                            </v-row>
                        </v-col> -->
                    </v-row>
                </v-card>
                ...
                <v-card>
                    <v-row>
                        <v-col md="12" class="py-0">
                            <DataGridView ref="grdDetail01" column-resizing-mode="widget" sel_procedure="SW_SEL_SWSO012"
                                upd_procedure="SW_UPD_SWSO012_v01" :auto_load="false" @setDataSource="onDataSourceSet"
                                @cellClick="onCellClick" :max_height="gridHeight"
                                :update_paras="['PK', 'CONFIRM_DELI_DT', 'STATUS']" select_mode="Multiple"
...
export default {
    ...
    computed: {
        ...
        gridHeader() {
            return [
                ...
                { dataField: 'MODEL', caption: this.$t('model'), allowEditing: false, width: 100 },
    ...
    methods: {
        ...
        async onReport() {
            ...
            let excel = [];
            let report = "SW_RPT_HAEIN_SWSO012";
