            ...
            <v-card>
                <v-container fluid class="pt-0">
                    <v-row dense class="pa-1">
                        ...
                        <v-col cols="2">
                            <BaseInput :label="$t('PO_NO/STYLE_COLOR')" v-model="txtAccCD_NM" @keyPressEnter="search"> 
    ...
    <v-row>
        <v-col md="4" cols="4" class="align-self-center">
            <v-btn-toggle dense mandatory :color="currentTheme" v-model="selectedTab">
                ...
                <v-btn value="tabMCL">{{ $t("COLOR") }}</v-btn>
                <v-btn value="tabSeason">{{ $t("MATERIAL") }}</v-btn> 
...
export default {
    ...
    components: {
        ...
        tabOrder: () => import("@/components/part/sw/10/SW10110_Order_FnF"),
        tabMCL: () => import("@/components/part/sw/10/SW10110_MCL_FnF"),
        tabSeason: () => import("@/components/part/sw/10/SW10110_Season_FnF"),
        tabTranslate: () => import("@/components/part/sw/10/SW10110_Translate"),
        tabExport: () => import("@/components/part/sw/10/SW10110_ExportTemChina_FnF"),
    ...
    watch: {
        selectedTab(val) {
            ...
            if (val == "tabOrder") {//ORDER
					...
					this.imp_store_prod = 'SW_IMP_SW10110_ORDER_FNF';
            ...
            if (val == "tabMCL") {//MCL
                ...
                this.imp_store_prod = 'SW_IMP_SW10110_COLOR_FNF';
            }
			if (val == "tabSeason") {//SEASON
                ...
                this.imp_store_prod = 'SW_IMP_SW10110_MAT_FNF';
            }
      ...
      this.LengthUpper = lstCommoncode[0];
		...
		async OnDeleteMaster() {
			  let tab = 4;
				   ...
				   const dso = {
					type: "process",
					updpro: "SW_PRO_SW10110_DELETE_FNF",
					para: [this.selectedCompany, this.month.replace("-", ""), tab, this.lstFactory]
	...
	async onReport() { 
		...
		const data_seach = [{
						...
						/*P_RPT_ID: "rpt6060010",
						P_RPT_URL: "report/60/60",
						
						P_COMPANY: this.mstData.TCO_COMPANY_PK,
						P_M_PK: this.mstData.PK,
						P_BOOK_CCY: this.mstData.TR_CCY,
						P_FILE_TYPE: ".xlsx",
						P_TEI_COMPANY_PK : this.mstData.TEI_COMPANY_PK*/
					}, ];
					var file_nm =
	...
	let report = "SWSO110_Order_Export_Size_CSV";
      try{
        this._ExcelSaveAs(await this.$axios.$get('/dso/makereport', {responseType: "blob", params: {
          ...
          excel: JSON.stringify([{
            sheet: 1,               
            insertRange: [{
              range: "A1:AJ1", proc: "SW_SEL_SW10110_SIZE_AU_NOCACHE", params: [...p_param], dateColumns: [ "OGAC" ]//SW_SEL_SW10110_export_custom
            }],
            insertRows: [{
				startRow: 1, proc: "SW_SEL_SW10110_SIZE_AU_NOCACHE", params: [...p_param], stringColumns: [ "SKU","CHN_NAME"], subStyle: { font : {   bold: true   } , fill: {
                                        type: 'pattern',
                                        pattern:'solid',
                                        fgColor : { argb: 'FFE599'},
                                        bgColor : { argb: 'FFE599'}
