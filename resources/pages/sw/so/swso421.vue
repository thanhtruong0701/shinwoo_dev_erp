    ...
    <v-card class="pa-1 pt-0">
      <v-row align="center" justify="space-between" class="pt-2">
        <v-col md="2" class="pt-0">
          <BaseSelect :label="$t('Customer')" v-model="tab01.lstCompany" :lstData="dataList.lstCompany"
            :text_all="$t('select_all')" item-text="NAME" item-value="CODE" />
        </v-col>
        <v-col md="2" class="pt-0">
          <BaseSelect :label="$t('Location')" v-model="tab01.lstLocation" :lstData="dataList.lstLocation"
            item-text="TEXT" item-value="PK" />
        </v-col>
        <v-col md="1" class="pt-0">
          <BaseSelect :label="$t('Date Type')" v-model="tab01.selectedDateType" :lstData="dataList.selectedDateType"
            item-text="TEXT" item-value="PK" />
        </v-col>
        <v-col md="1" class="pt-0">
        ...
        <v-col md="1" class="pt-0">
          <v-text-field clearable dense hide-details :label="$t('Container')" @keypress.enter="OnSearch(1)"
            v-model="tab01.txtOrderNo"></v-text-field>
        </v-col>
        <v-col md="2" class="pt-0">
          <BaseSelect :label="$t('Label_Type')" v-model="tab01.lstLabelType" :lstData="dataList.lstLabelType"
            :text_all="$t('select_all')" item-text="NAME" item-value="CODE" />
        </v-col>
      </v-row>
      ...
      <v-row align="center" justify="space-between" class="pt-0">
        <v-col md="2" class="pt-0">
          <v-row no-gutters>
            ...
            <v-col cols="auto">
              <BaseButton btn_type="icon" icon_type="set" :btn_text="$t('set-container')" :disabled="isProcessing"
                @onclick="$refs.grdDetail11.setCellSelected('CONTAINER_NAME', tab01.txtContainer)" class="ml-2" />
            </v-col>
          </v-row>
        </v-col>
        <v-col md="2" class="pt-0">
          <v-row no-gutters>
            ...
            <v-col cols="auto">
              <BaseButton btn_type="icon" icon_type="set" :btn_text="$t('set-CBM')" :disabled="isProcessing"
                @onclick="$refs.grdDetail11.setCellSelected('CBM', tab01.txtCMB)" class="ml-2" />
            </v-col>
          </v-row>
        </v-col>
        <v-col md="2" class="pt-0">
          <v-row no-gutters>
            ...
            <v-col cols="auto">
              <BaseButton btn_type="icon" icon_type="set" :btn_text="$t('set-UNIT')" :disabled="isProcessing"
                @onclick="$refs.grdDetail11.setCellSelected('UNIT', tab01.txtUnit)" class="ml-2" />
            </v-col>
          </v-row>
        </v-col>
        <v-col md="3" class="pt-0">
          <v-row no-gutters>
            <v-col>
              <BaseSelect :label="$t('LOCATION')" v-model="tab01.txtDESCRIPTION" :lstData="dataList.txtDESCRIPTION"
                item-text="TEXT" item-value="PK" />
            </v-col>
            <v-col cols="auto">
              <BaseButton btn_type="icon" icon_type="set" :btn_text="$t('set-LOCATION')" :disabled="isProcessing"
                @onclick="$refs.grdDetail11.setCellSelected('DESCRIPTION', tab01.txtDESCRIPTION)" class="ml-2" />
            </v-col>
          </v-row>
        </v-col>
        <v-col md="3" class="pt-0">
          <v-row no-gutters>
            ...
            <v-col cols="auto">
              <BaseButton btn_type="icon" icon_type="set" :btn_text="$t('set-DELIVERY-DATE')" :disabled="isProcessing"
                @onclick="$refs.grdDetail11.setCellSelected('DELIVERY_DATE_SW', tab01.dtDelivery_SW)" class="ml-2" />
            </v-col>
          </v-row>
        </v-col>
      </v-row>

      <!-- Row for remaining buttons -->
      <v-row justify="end" class="mt-0">
        <v-col cols="12" class="d-flex justify-end">
          <BaseButton icon_type="search" :btn_text="$t('search')" :disabled="isProcessing" @onclick="OnSearch(1)"
            class="mr-2" />
          <BaseButton icon_type="add" :btn_text="$t('AddNew')" :disabled="isProcessing" @onclick="onAdd()"
            class="mr-2" />
          <BaseButton icon_type="save" :btn_text="$t('save')" :disabled="isProcessing" @onclick="OnSave()"
            class="mr-2" />
          <BaseButton icon_type="delete" :btn_text="$t('DELECT')" :disabled="isProcessing" @onclick="onDelete()"
            class="mr-2" />
          ...
          <BaseButton icon_type="excel" :btn_text="$t('Container')" :disabled="isProcessing" @onclick="onReport1"
            class="mr-2" />
          <BaseButton icon_type="excel" :btn_text="$t('excel-2')" :disabled="isProcessing"
            @onclick="onReportDeliMerge" />
        </v-col>
      </v-row>
    </v-card>
    ...
    <v-card class="pt-0">
      <v-row align="center" justify="space-between" class="pt-0">
        <v-col cols="12" class="pt-0">
          <DataGridView column-resizing-mode="widget" ref="grdDetail11" select_mode="Multiple" :autocheckbox="false"
            :data-source="gridData" :columns="dataList.headerGrid11" :auto_load="false" :gridfilter='false'
            @cell-value-changed="onCellValueChanged" :max_height="this.windowHeight * 0.65"
            sel_procedure="SW_SEL_SWSO421_01" upd_procedure="SW_UPD_SWSO421_01" @onRowPrepared="onRowPreparedGrid11"
            :filter_paras="[
              tab01.lstCompany,
              tab01.lstLocation,
...
export default {
  ...
  async created() {
    ...
    [this.dataList.lstCompany, this.dataList.lstLabelType] = await this._getCommonCode2(["SZ008", "SWSO421"]);
    //[this.dataList.lstLabelType] = await this._getCommonCode2(["SZ008"]);
    ...
    //this.dataList.lstLabelType = [{ PK: '', TEXT: 'ALL' }, { PK: '01', TEXT: '단독세탁라벨KK-Caution Label' }, { PK: '02', TEXT: '의류라벨EC-EC Carelabel' }, { PK: '03', TEXT: '의류라벨KK-Korean Carelabel' }, { PK: '04', TEXT: '합격증EC-Price Tag - China' }, { PK: '05', TEXT: '의류가격택1KK-Price Tag - Global' }, { PK: '06', TEXT: "GM-02 LABEL" }, { PK: '07', TEXT: 'TAG STRING' }];
  ...
  methods: {
    onAdd() {
      const selectedCompany = this.dataList.lstCompany.find(company => company.CODE === this.tab01.lstCompany);
      const selectedLabelType = this.dataList.lstLabelType.find(labelType => labelType.CODE === this.tab01.lstLabelType);
      if (!selectedCompany) {
        this.showNotification("warning", this.$t("Please select a Customer other than 'Select All'"), "", 4000);
        return;
      }
      if (this.tab01.lstLabelType !== '06' && this.tab01.lstLabelType !== '07') {
        this.showNotification("warning", this.$t("Please select Label Type as 'GM-02 LABEL' or 'TAG STRING'"), "", 4000);
        return;
      }
      ...
      this.$refs.grdDetail11.addRowStruct({
        ...
        CONTAINER_NAME: '',
        CONTAINER_NAME_02: '',
        CUST_NAME_ENG: selectedCompany.NAME,
        ORDERDATE: '',
        Z_REQ_DELIVERY_DATE: '',
        DELIVERY_DATE_SW: '',
        ORDERNO: '',
    ...
    //     {
    //   let arrDataSource = this.$refs.grdDetail11.getDataSource();
    //   for (let i = 0; i < arrDataSource.length; i++) {
    //     if (
    //       !this._hasValue(arrDataSource[i].ORDERDATE) ||
    //       !this._hasValue(arrDataSource[i].ORDERDATE)
    //     ) {
    //       this.showNotification("warning", this.$t("Please enter data for 'Order_date'"), "", 4000);
    //       return false;
    //     }
    //     if (
    //       !this._hasValue(arrDataSource[i].ORDERNO) ||
    //       !this._hasValue(arrDataSource[i].ORDERNO)
    //     ) {
    //       this.showNotification("warning", this.$t("Please enter data for'Order_no'"), "", 4000);
    //       return false;
    //     }
    //     if (
    //       !this._hasValue(arrDataSource[i].STYLE_NO) ||
    //       !this._hasValue(arrDataSource[i].STYLE_NO)
    //     ) {
    //       this.showNotification("warning", this.$t("Please enter data for'Style_no'"), "", 4000);
    //       return false;
    //     }
    //   }
    //   return true;
    ...
    OnValidate() {
      ...
      // Kiểm tra tất cả các hàng, không chỉ hàng vừa được chỉnh sửa
      for (let i = 0; i < arrDataSource.length; i++) {
        let row = arrDataSource[i];

        if (!this._hasValue(row.ORDERDATE)) {
          this.showNotification("warning", this.$t("Please enter data for 'Order_date'"), "", 4000);
          return false;
        }

        if (!this._hasValue(row.ORDERNO)) {
          this.showNotification("warning", this.$t("Please enter data for 'Order_no'"), "", 4000);
          return false;
        }

        if (!this._hasValue(row.STYLE_NO)) {
          this.showNotification("warning", this.$t("Please enter data for 'Style_no'"), "", 4000);
          return false;
        }
      }
    ...
    processHeader1(id) {
      ...
      this.dataList.headerGrid01.push({ dataField: "ZZZZ", caption: "-", allowEditing: true });
      switch (id) {
        case 1:
    ...
    async OnSave() {
      // Đợi một chút để đảm bảo grid đã cập nhật dữ liệu
      await new Promise(resolve => setTimeout(resolve, 0));

      if (this.OnValidate()) {
        try {
          await this.$refs.grdDetail11.saveData();
          this.showNotification("success", this.$t("Data saved successfully"), "", 4000);
        } catch (error) {
          console.error("Error saving data:", error);
          this.showNotification("error", this.$t("Error saving data"), "", 4000);
        }
      }
    },
    ...
    {PK: '03', TEXT: '의류라벨KK-Korean Carelabel'},  {PK: '04', TEXT: '합격증EC-Price Tag - China'}, {PK: '05', TEXT: '의류가격택1KK-Price Tag - Global'}];*/
      let report = "";
      if (this.tab01.lstLabelType === "01" || this.tab01.lstLabelType === "02" || this.tab01.lstLabelType === "03" || this.tab01.lstLabelType === "04" || this.tab01.lstLabelType === "05") {
        report = "rpt_swso421__delivery";
      }
      if (this.tab01.lstLabelType === "06" || this.tab01.lstLabelType === "07") {
        report = "rpt_swso421__delivery_hand";
      }

      if (this.tab01.lstLabelType === "") {
        report_name = "rpt_swso421_delivery_ALL_Label";
      }
      if (this.tab01.lstLabelType === "01") {
        report_name = "rpt_swso421_delivery_Caution_Label";
      }
      if (this.tab01.lstLabelType === "02") {
        report_name = "rpt_swso421_delivery_EC_Carelabel";
      }
      if (this.tab01.lstLabelType === "03") {
        report_name = "rpt_swso421_delivery_Korean_Carelabel";
      }
      if (this.tab01.lstLabelType === "04") {
        report_name = "rpt_swso421_delivery_Price_Tag_China";
      }
      if (this.tab01.lstLabelType === "05") {
        report_name = "rpt_swso421_delivery_Price_Tag_Global";
      }
      if (this.tab01.lstLabelType === "06") {
        report_name = "rpt_swso421_Price_Tag_China_Normal_06";
      }
      if (this.tab01.lstLabelType === "07") {
        report_name = "rpt_swso421_Price_Tag_Global_RFID_07";
      }
        ...
        this._ExcelSaveAs(await this.$axios.$get('/dso/makereport', {
          responseType: "blob", params: {
            ...
            excel: JSON.stringify([{
              insertRange: [{
                range: "A1:AJ2", proc: "SW_SEL_SWSO421_01", params: [...p_param]//SW_SEL_SW10110_export_custom
              }],
              insertRows: [{
                startRow: 8, proc: "SW_SEL_SWSO421_01", params: [...p_param], stringColumns: ["CONTAINER_NAME", "ORDERNO"], subStyle: {
                  font: { bold: true }, fill: {
                    type: 'pattern',
                    pattern: 'solid',
                    fgColor: { argb: 'FFE599' },
        ...
        this._ExcelSaveAs(await this.$axios.$get('/dso/makereport', {
          responseType: "blob", params: {
            ...
            excel: JSON.stringify([{
              ...
              insertRows: [{
                startRow: 7, proc: "SW_SEL_SWSO420_RPT_CONTAINER", params: [...p_param2], stringColumns: ["CONTAINER_NAME", "ORDERNO"], subStyle: {
                  font: { bold: true }, fill: {
                    type: 'pattern',
                    pattern: 'solid',
                    fgColor: { argb: 'FFE599' },
