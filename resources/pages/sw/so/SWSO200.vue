                ...
                <v-container fluid class="pt-0">
                    <v-row dense class="pa-1">
						...
						<!--BaseDatePicker  v-model="selectedReceiveFrom" outlined  :label="$t('receive_dt_from')"  ></BaseDatePicker-->
                            <BaseDatePicker v-model="month" outlined :label="$t('date_from')" default></BaseDatePicker> 
                        </v-col>
						<v-col cols="2">
						<!--BaseDatePicker  v-model="selectedReceiveFrom" outlined  :label="$t('receive_dt_from')"  ></BaseDatePicker-->
                            <BaseDatePicker v-model="month2" outlined :label="$t('date_to')" default></BaseDatePicker> 
                        </v-col>
                        <v-col cols="2">
    ...
    <v-row>
        <v-col md="4" cols="4" class="align-self-center">
            <v-btn-toggle dense mandatory :color="currentTheme" v-model="selectedTab">
                <v-btn value="tabOrder">{{ $t("Order Entry") }}</v-btn>
...
export default {
    ...
    components: {
        ...
        tabOrder: () => import("@/components/part/sw/10/SW10200_OrderEntryImport"),
    ...
    data: () => ({
		...
		imp_start_row:4,
        impEndCol: 11,
        imp_template_file: 'report/SW/10/RPT_IMP_ORDER_ENTRY_SW.xlsx',// RPT_IMP_ORDER_TAIWAN
        imp_store_prod: 'SW_IMP_SW10200_ORDER_ENTRY',
		tab_staus:1,
    }),
    async created() {
    ...
    watch: {
        selectedTab(val) {
            if (val == "tabOrder") {//ORDER
                    ...
                    this.tab_staus=1;
					if(this.selectedCompany=='6')
					{
						//alert(this.lstFactory);
						this.impEndCol = 11;
						this.imp_start_row=4;
						this.imp_template_file = 'report/SW/10/RPT_IMP_ORDER_ENTRY_SW.xlsx';
						this.imp_store_prod = 'SW_IMP_SW10200_ORDER_ENTRY';
					} 
					...
					if(this.lstFactory=='VE')
					{
						//alert(this.lstFactory);
						this.impEndCol = 76;
						this.imp_start_row=2;//INSERT HEADER SIZE
