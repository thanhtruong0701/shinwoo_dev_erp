...
class rpt5070020_v01 {
    async rpt5070020({ request, response, auth }) {
        try {
            ...
            if (!user) {
                ...
                var dtDetail = await DBService.callProcCursor("LG_SEL_5070020_SEARCH", [objParam.p_from_date,objParam.p_date_to], p_language, user.USER_ID) 
                if (dtDetail) {
                    dtDetail = dtDetail
                } else {
                ...
                const templateFile = Helpers.resourcesPath('report/50/70/rpt_5070020_v01.xlsx')
                ...
                rowHeader.getCell(3).value = dtDetail[0].PARE_DATE;
                //rowHeader.commit();
                var pos = 5;
                ...
                for (var i = 0; i < dtDetail.length; i++) {
                    ...
                    row.getCell(2).value = dtDetail[i].INSPECTION_DT;
                    row.getCell(3).value = dtDetail[i].SLIP_NO;
                    row.getCell(4).value = dtDetail[i].AR_PARTNER_NM;
                    row.getCell(5).value = dtDetail[i].ITEM_CODE;
                    row.getCell(6).value = dtDetail[i].ITEM_NAME;
                    row.getCell(7).value = dtDetail[i].DEFECT_QTY;
                    row.getCell(8).value = dtDetail[i].UNIT_PRICE;
                    row.getCell(9).value = dtDetail[i].DC_AMOUNT;
                    row.getCell(10).value = dtDetail[i].AMOUNT;
                    row.getCell(11).value = dtDetail[i].AR_AMOUNT;
                    /*if(i>0)
                    {
                ...
                worksheet.getCell(pos+total_rows+1,11).border = {bottom: {style:'medium'}};
                worksheet.getCell(pos+total_rows+1,1).border = {bottom: {style:'medium'}};

                worksheet.getCell(pos+total_rows-1,12).border = {right: {style:'medium'}};
