...
class rptSH20090 {
    async rpt_SH20090({ request, response, auth }) {
        try {
            ...
            if (!user) {
                ...
                const dt = await DBService.callProcCursor("SP_RPT_SH20090_NOCACHE", para_dt, language, crt_by);
                if (dt.length == 0) {
                ...
                let date = new Date();
                worksheet.getCell("B2").value = "Period: " + from_date.substr(0,4) + "." + from_date.substr(4,2) + "." + from_date.substr(6,2) + " - " + to_date.substr(0,4) + "." + to_date.substr(4,2) + "." + to_date.substr(6,2);
                worksheet.getCell("I2").value = "Date: " + date.getFullYear() + "." + (date.getMonth()+1).toString().padStart(2, "0") + "." + date.getDate().toString().padStart(2, "0");
