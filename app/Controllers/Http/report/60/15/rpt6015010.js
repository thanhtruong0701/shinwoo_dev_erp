...
class rpt6015010 {
    /*########################################################## Report JH ##################################################################################*/
    async rpt_6015010_T2({ request, response, auth }) {
        try {
            ...
            var _store = "AC_RPT_6015010_ACC_INQUIRY";
            var _param = [item.P_COMPANY, item.P_ACC_CODE, item.P_ACC_NAME, item.P_USE_YN,];
            /***************************** Return failded ****************************/
            if (!user) {
            ...
            /****************************** Begin call store and write excell *********/
                ...
                var cnt = 0;
                var total_header = (Number(dt_Data[0].TOTAL_COL) + 15);
                ...
                for (var i = 0; i < dt_Data.length; i++) {
                    ...
                    row_item.getCell(9).value = dt_Data[i].CARRY_YN;

                    row_item.getCell(10).value = dt_Data[i].AC_LEVEL;
                    row_item.getCell(11).value = dt_Data[i].CUST_YN;
                    row_item.getCell(12).value = dt_Data[i].CUST_REMYN;
                    row_item.getCell(13).value = dt_Data[i].PL_YN;
                    row_item.getCell(14).value = dt_Data[i].PL_TYPE;
                    row_item.getCell(15).value = dt_Data[i].PL_TYPENM;
                    var count = 16;
                    for (var j = 0; j < arr_header_D.length; j++) {
            ...
            Utils.Logger({ LVL: "error", MODULE: "Report", FUNC: "rpt_6015010_T2", CONTENT: e.message })
