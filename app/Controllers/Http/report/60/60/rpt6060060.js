    ...
    async report1({ request, response, auth }) {
        try {
            ...
            if (!user) {
                ...
                const templateFile = Helpers.resourcesPath('report/60/60/6060060.xlsx')
                let reportPath = `6060060_${user.USER_ID}.xlsx`
               ...
               // let _AccCode = dtDetail.length > 0 ? dtDetail[0].AC_CD : "";
                ...
                for (let i = 0; i < dtDetail.length; i++) {
                    ...
                    row.getCell(2).value = dtDetail[i].PO_NO;
                    row.getCell(3).value = dtDetail[i].ITEM_CODE;
                    row.getCell(4).value = dtDetail[i].ITEM_NAME;
                    row.getCell(5).value = dtDetail[i].SPEC;
                    row.getCell(6).value = dtDetail[i].QTY;

                    row.getCell(7).value = dtDetail[i].U_PRICE;
                    row.getCell(8).value = dtDetail[i].AMOUNT;
                    row.getCell(9).value = dtDetail[i].DISCOUNT_AMT;
                    row.getCell(10).value = dtDetail[i].VAT_AMT;
                    row.getCell(11).value = dtDetail[i].DEBIT_AMT;
                    row.getCell(12).value = dtDetail[i].CREDIT_AMT;
                    row.getCell(13).value = dtDetail[i].BAL_AMT;
                    ...
                    if (dtDetail[i].BOLD_YN == "Y") {
                        ...
                        worksheet.mergeCells(_rPO_start_merg, 2, _rPO_end_merg - 1, 2);
                ...
                return response.attachment(
                    reportFile,
                    `rpt6060060_${user.USER_ID}.xlsx` // custom name
                )
            }
        } catch (e) {
