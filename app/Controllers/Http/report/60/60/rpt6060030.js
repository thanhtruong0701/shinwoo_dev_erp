    ...
    async report1({ request, response, auth }) {
        try {
            ...
            if (!user) {
                ...
                let dtMaster = await DBService.callProcCursor("AC_RPT_6060030_MST", para, p_language, user.USER_ID)
                let dtDetail = await DBService.callProcCursor(proc, para, p_language, user.USER_ID)
                const templateFile = Helpers.resourcesPath('report/60/60/6060030.xlsx')
                let reportPath = `6060030_${user.USER_ID}.xlsx`
                const reportFile = Helpers.tmpPath(`6060030_${user.USER_ID}.xlsx`)
            ...
            Utils.Logger({ LVL: "error", MODULE: "6060030", FUNC: "6060030", CONTENT: e.message })
