...
class rpt6050130 {
/*########################################################## Report JH ##################################################################################*/ 
        async rpt_6050130_PO_AP_DETAILS_INQ({ request, response, auth }) {
            try 
            { 
            /*  P_RPT_ID: report_info.VAL1, P_RPT_URL: report_info.VAL2, P_RPT_FILE: report_info.VAL3,
                ...
                P_COMPANY: this.selectedCompany, P_FR_MONTH: this.Month_fr, P_TO_MONTH: this.Month_to, P_PARTNER_PK : this.txtCust_Pk, 
                P_PL_PK : this.txtPL_PK,P_ACC_PK: this.TAC_ABACCTCODE_PK, P_SLIP_NO: this.txtSlipNo, P_PO_NO : this.txtPONo,P_ITEM_CODE: this.txtItem
                */
                /****************************** Get Param *********************************/
                ...
                var _store          = "AC_SEL_6050130_INQ";
               // var _circular       = "AC_SEL_GET_TT_BPL";
                var _param           = [item.P_COMPANY, item.P_TCO_BUSPLACE_PK,  item.P_FR_MONTH, item.P_TO_MONTH, item.P_PARTNER_PK, item.P_PL_PK, item.P_ACC_PK, item.P_SLIP_NO, item.P_PO_NO, item.P_ITEM_CODE];
                ...
                { 
                   ...
                   /* var dt_TT  = await DBService.callProcCursor(_circular, _param_TT , p_language , p_crt_by); 
                    ...
                    for (var i = 0; i < dt_Data.length; i++) 
                    {
                        ...
                        row_item.getCell(1).value    =  dt_Data[i].SLIP_NO;
                        row_item.getCell(2).value    =  dt_Data[i].REF_NO;
                        row_item.getCell(3).value    =  dt_Data[i].TAKEIN_DATE;
                        row_item.getCell(4).value    =  dt_Data[i].ITEM_CODE;
                        row_item.getCell(5).value    =  dt_Data[i].ITEM_NAME;
                        row_item.getCell(6).value    =  dt_Data[i].INV_ITEM_UOM;
                        row_item.getCell(7).value    =  Number(dt_Data[i].INPUT_QTY);
                        row_item.getCell(8).value    =  Number(dt_Data[i].ITEM_AMOUNT);
                        row_item.getCell(9).value    =  Number(dt_Data[i].AP_QTY);
                        row_item.getCell(10).value   =  dt_Data[i].VOUCHERNO;
                        row_item.getCell(11).value   =  dt_Data[i].PARTNER_ID;
                        row_item.getCell(12).value   =  dt_Data[i].PARTNER_NAME;
                        row_item.getCell(13).value   =  dt_Data[i].CREATE_BY;
                        pos = pos + 1; 

                        
                        total_input_qty       += Number(dt_Data[i].INPUT_QTY);
                        total_item_amount     += Number(dt_Data[i].ITEM_AMOUNT);
                        //row_item.commit();
                    ...
                    row.getCell(7).value    = total_input_qty ;
                    row.getCell(8).value    = total_item_amount;
            ...
            {
                Utils.Logger({ LVL: "error", MODULE: "Report", FUNC: "rpt_6050130_PO_AP_DETAILS_INQ", CONTENT: e.message })
