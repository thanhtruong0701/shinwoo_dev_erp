...
class rpt6025040 {
/*########################################################## Report JH ##################################################################################*/ 
    async rpt_6025040_PB({ request, response, auth }) {
        try 
        { 
...
// sign
                ...
                if( P_FILE_TYPE ==".xlsx")
                {  
                    return response.attachment( reportFile, file_new+ P_FILE_TYPE );
                }

                if( P_FILE_TYPE ==".pdf")
                { 
                    const reportFilePdf =  await Utils.excelToPdf(reportFile);
                    return response.attachment( reportFilePdf, file_new+ P_FILE_TYPE );
                }
