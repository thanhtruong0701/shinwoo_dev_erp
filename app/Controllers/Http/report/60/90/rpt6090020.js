...
class rpt6090020 {
/*########################################################## Report JH ##################################################################################*/ 
    async rpt_6090020_LIST({ request, response, auth }) {
        try 
        { 
          /*     P_RPT_ID:   report_info.VAL1,
                                ...
                                P_POSUM:    this.sel_SumPos, 
                                P_DT_FROM:  this.dt_from, 
                                P_DT_TO:    this.dt_to, 
                                P_OPTION:   this.sel_Option, 
                                P_ACC_CD_FR: this.txt_AccCodeFr, 
                                P_ACC_CD_TO: this.txt_AccCodeTo, 
            ...
            var _store          = "AC_RPT_6090020_NOCACHE";
            var _param          = [ item.P_COMPANY, item.P_BIZ, item.P_POSUM, item.P_DT_FROM, item.P_DT_TO, item.P_OPTION, item.P_ACC_CD_FR, item.P_ACC_CD_TO, item.P_STATUS, item.P_LANGUAGE]; 
             /***************************** Return failded ****************************/
            if (!user) 
            {
            ...
            { 
                ...
                row_item.getCell(2).value = item.P_BIZ_NM;   // vng--304

                    row_item = worksheet.getRow(2);
                    row_item.getCell(2).value = item.P_BIZ_ADD;

                    row_item = worksheet.getRow(4);
                    row_item.getCell(7).value = Utils._formatDate(item.P_DT_FROM,"DD-MM-YYYY")  + " ~ " +  Utils._formatDate(item.P_DT_TO,"DD-MM-YYYY") ;  
                ...
                for (var i = 0; i < dt_Data.length; i++) 
                {
                    ...
                    row_item.getCell(4).value  = dt_Data[i].CODES_OF_ACC;
                                                
                    row_item.getCell(5).value  = dt_Data[i].NAME_OF_ACC
                    row_item.getCell(6).value  = dt_Data[i].ACC_CD;
                    row_item.getCell(7).value  = dt_Data[i].ACC_NM;
                    row_item.getCell(8).value  = dt_Data[i].DESCRIPTION;
                    row_item.getCell(9).value  = dt_Data[i].DESCRIPTION_LOCAL;
                                                
                    row_item.getCell(10).value = Number(dt_Data[i].DEBIT_AMT);
                    row_item.getCell(11).value = Number(dt_Data[i].CREDIT_AMT);
                    row_item.getCell(12).value = Number(dt_Data[i].DEBIT_BAL_AMT);
                    row_item.getCell(13).value = Number(dt_Data[i].CREDIT_BAL_AMT);
                    row_item.getCell(14).value = Number(dt_Data[i].DEBIT_FC_AMT);
                                                
                    row_item.getCell(15).value = Number(dt_Data[i].CREDIT_FC_AMT);
                    row_item.getCell(16).value = Number(dt_Data[i].DEBIT_FC_BAL_AMT);
                    row_item.getCell(17).value = Number(dt_Data[i].CREDIT_FC_BAL_AMT);
                    row_item.getCell(18).value = dt_Data[i].SEQ;
                    row_item.getCell(19).value = dt_Data[i].INVOICE_DATE
                                                
                    row_item.getCell(20).value = dt_Data[i].INVOICE_NO;
                    row_item.getCell(21).value = dt_Data[i].DEBIT_PARTNER;
                    row_item.getCell(22).value = dt_Data[i].CREDIT_PARTNER; 
    ...
    async rpt_6090020_SHEET ({ request, response, auth }) {
        try 
        { 
           /*     P_RPT_ID:   report_info.VAL1,
                                P_RPT_URL:  report_info.VAL2,
            ...
            var file_new        = file_nm + p_crt_by + file_type;
            var _resourcesPath  = [item.P_RPT_URL]+'/'+full_nm;
            var _store          = "AC_RPT_6090020_ACC_ALL";
            var _store_confirm  = "AC_RPT_6045035_DTL_ACC_CODE_V2";
            var _store_approve  = "AC_RPT_6045035_APPROVAL_2";
            var _circular       = "AC_SEL_GET_TT_BPL";
            var _param          = [item.P_COMPANY, item.P_DT_FROM, item.P_DT_TO, item.P_ACC_CD_FR, item.P_ACC_CD_TO, item.P_STATUS, item.P_POSUM, item.P_BIZ];
            var _param_TT       = [item.P_CIRCULARS, item.P_BIZ];
            /***************************** Return failded ****************************/
            if (!user) 
            {
                return response.send(Utils.response(false, "Request failed!", null));
            ...
            { 
                ...
                var p_ps_dr = 0;
                var p_ps_cr=0; 
                var p_option = item.P_OPTION ;
                var l_count = 0 ;
               for (var i = 0; i < dt_TK.length; i++) 
                {

                    //var row_item = worksheet.getRow(pos); 
                    account_pk      = dt_TK[i].PK;    
                    p_open_dr       = dt_TK[i].OPEN_DR;
                    p_open_cr       = dt_TK[i].OPEN_CR;    
                    p_ps_dr         = dt_TK[i].PS_DR;  
                    p_ps_cr         = dt_TK[i].PS_CR; 
                    account_code    = dt_TK[i].AC_CD; 
                    var _param_1    = [item.P_COMPANY, item.P_DT_FROM, item.P_DT_TO, account_pk, p_open_dr ,p_open_cr, '', '' ,item.P_STATUS, item.P_POSUM, dt_TK[i].DRCR_TYPE, item.P_BIZ];
                   ...
                   if(p_open_dr !=0 || p_open_cr != 0 || p_ps_dr != 0|| p_ps_cr != 0)
                    {
                        let worksheet       = workbook.addWorksheet(); 
                        worksheet.model = worksheet_1.model;
                        l_count = 1 ;
                        worksheet.name = dt_TK[i].AC_CD; 
                      
                        //Merge
                        worksheet.mergeCells(4,1,4,12);// Title
                        worksheet.mergeCells(5,1,5,12); //from - to date
                        worksheet.mergeCells(10,1,11,1); //date
                        worksheet.mergeCells(10,2,10,3); //voucher 
                        worksheet.mergeCells(10,4,11,4); //local description
                        worksheet.mergeCells(10,5,11,5); // description

                        worksheet.mergeCells(10,6,11,6); // Account Code
                        worksheet.mergeCells(10,7,11,7); // Account Name
                        worksheet.mergeCells(10,8,11,8); // Corresponding
                        worksheet.mergeCells(10,9,11,9); // Corresponding Name

                        worksheet.mergeCells(10,10,10,11); //Amount 
                        worksheet.mergeCells(10,12,10,13); //Balance 
                        worksheet.mergeCells(10,14,11,14); //remark 
                        ...
                        worksheet.getCell("A5").value = dt_TK[i].DT_PARA_ENG;
                        worksheet.getCell("M8").value = _bookccy;
                        //thÃ´ng tÆ°
                        var _lang = 'ENG';
                        if(_lang == 'VIE'){
                        ...
                        worksheet.getCell("B6").value = dt_TK[i].AC_CD;
                        worksheet.getCell("C6").value = dt_TK[i].AC_NM;
                        let l_status;
                        if(item.P_STATUS =="2"){
                            l_status ="Confirmed";
                        }else{
                        ...
                        worksheet.getCell("B7").value = l_status;
                        var _print = '2';
                        if(_print == '2'){
                            worksheet.getCell("A10").value ="Date";
                        }
                        else{
                            worksheet.getCell("A10").value ="Approve No" ;
                        }
                        //-------Fill Data----------------
                        worksheet.getCell("L13").value = dt_TK[i].OPEN_DR;
                        worksheet.getCell("M13").value = dt_TK[i].OPEN_CR;
                        worksheet.getCell("J15").value = dt_TK[i].PS_DR;
                        worksheet.getCell("K15").value = dt_TK[i].PS_CR;
                        worksheet.getCell("L16").value = dt_TK[i].END_DR;
                        worksheet.getCell("M16").value = dt_TK[i].END_CR;
                        var pos = 14; 
                        if(dt_Data.length>1)
                        {
                             worksheet.duplicateRow(pos,dt_Data.length-1,true);
                       ...
                       for (var y = 0; y < dt_Data.length; y++) 
                        {
                            ...
                            row_item.getCell(3).value   = dt_Data[y].TRANS_DATE2;  
                            row_item.getCell(4).value   = dt_Data[y].REMARK2; 
                            row_item.getCell(5).value   = dt_Data[y].REMARK; 
                            row_item.getCell(6).value   = dt_Data[y].AC_CD;
                            row_item.getCell(7).value   = dt_Data[y].AC_NM;
                            row_item.getCell(8).value   = dt_Data[y].ACCT_CODE; 
                            row_item.getCell(9).value   = dt_Data[y].ACCT_NAME; 
                            row_item.getCell(10).value  = dt_Data[y].PS_DEBIT; 
                            row_item.getCell(11).value  = dt_Data[y].PS_CREDIT;  
                            row_item.getCell(12).value  = dt_Data[y].TOTAL_DEBIT;  
                            row_item.getCell(13).value  = dt_Data[y].TOTAL_CREDIT;  
                            row_item.getCell(14).value  = dt_Data[y].PK;
                            row_item.getCell(15).value  = dt_Data[y].TR_ENCLOSE;  
                            pos = pos + 1; 
                            //row_item.commit();
                        }  
        ...
        {
            Utils.Logger({ LVL: "error", MODULE: "Report", FUNC: "rpt_6090020_SHEET", CONTENT: e.message })
           // return response.send(Utils.response(false, e.message, null))
           return response.send(null);
        }
    }  
    async rpt_6090020_BANK_SHEET ({ request, response, auth }) {
        try 
        { 
           /*P_RPT_ID:   report_info.VAL1,
            P_RPT_URL:  report_info.VAL2,
            ...
            var file_new        = file_nm + p_crt_by + file_type;
            var _resourcesPath  = [item.P_RPT_URL]+'/'+full_nm;
            var _store_bank     = "AC_6090020_BANK_LIST";
            var _store_dt       = "AC_RPT_6045035_ACC_OPENING_BPL";
            var _store          = "AC_RPT_6045050_BANK_CUST_BPL";
            var _circular       = "AC_SEL_GET_TT_BPL";
            var _param_bank     = [item.P_COMPANY, item.P_ACC_CD_FR, item.P_ACC_CD_TO, item.P_BIZ];
            var _param_TT       = [item.P_CIRCULARS, item.P_BIZ];
               ...
               for (var l_bank = 0; l_bank < dt_Bank.length; l_bank++) 
                {
                    ...
                    //var row_item = worksheet.getRow(pos); 
                    var _param_dt    = [item.P_COMPANY, item.P_DT_FROM, item.P_DT_TO, _acc_pk, item.P_STATUS, bank_pk, item.P_BIZ];
                    ...
                    var _param    = [item.P_COMPANY, item.P_DT_FROM, item.P_DT_TO, _acc_pk ,item.P_STATUS, p_open_trans, p_open_books, bank_pk, bank_id, cust_pk, item.P_BIZ ];
                    var dt_Data  = await DBService.callProcCursor(_store, _param , p_language , p_crt_by); 
                    ...
                    if(dt_DT.length>0 || p_open_trans != 0 || p_open_books != 0)
                    {
                        ...
                        var FromDate = item.P_DT_FROM;
                        var ToDate = item.P_DT_TO;
        ...
        {
            Utils.Logger({ LVL: "error", MODULE: "Report", FUNC: "rpt_6090020_BANK_SHEET", CONTENT: e.message })
           // return response.send(Utils.response(false, e.message, null))
           return response.send(null);
        }
    }
