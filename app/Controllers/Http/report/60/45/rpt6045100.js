...
class rpt6045100 {
/*########################################################## Report JH ##################################################################################*/ 
    async rpt_6045100_TBNETOFF({ request, response, auth }) {
        try 
        { 
          /*P_RPT_ID: report_info.VAL1, P_RPT_FILE: report_info.VAL2,
            ...
            var _store          = "AC_SEL_6045100_01_NOCACHE";
            var _param          = [ item.P_COMPANY, item.P_LANG, item.P_FR_DT, item.P_TO_DT, item.P_STATUS, item.P_LEVEL, item.P_POSSUM, item.P_ACCD, item.P_ACGROUP, item.P_CCY, item.P_EX_RATE, item.P_BK_RATE, item.P_UNIT, item.P_TCO_BUSPLACE_PK];
             /***************************** Return failded ****************************/
            if (!user) 
            {
            ...
            { 
                ...
                { 
                    ...
                    r_item.getCell(1).value = Utils.translate("COMPANY",_dictionary , p_language) + ": " + item.P_BIZ_ID +" - "+item.P_BIZ_NM;    
                    ...
                    r_item.getCell(1).value = Utils.translate("TRIAL_BALANCE_NET_OFF",_dictionary , p_language)  ; 
                    ...
                    r_item.getCell(5).value = Utils.translate("OPENNING_BALANCE",_dictionary , p_language)  ; 
                    r_item.getCell(6).value = Utils.translate("PERIOD_AMT",_dictionary , p_language)  ; 
                    // r_item.getCell(9).value = Utils.translate("ACCOUNT_AMT",_dictionary , p_language)  ; 
                    r_item.getCell(8).value = Utils.translate("ENDING_BALANCE",_dictionary , p_language)  ; 
                    // r_item.getCell(12).value = Utils.translate("LEAF_YN",_dictionary , p_language)  ; 
                    ...
                    // r_item.getCell(9).value = Utils.translate("DEBIT",_dictionary , p_language)  ; 
                    // r_item.getCell(10).value = Utils.translate("CREDIT",_dictionary , p_language)  ; 
                    // r_item.getCell(11).value = '' ;  
                ...
                for (var i = 0; i < dt_Data.length; i++) 
                {
                   ...
                   // row_item.getCell(2).value = (p_language == 'ENG')?dt_Data[i].AC_NM:(p_language == 'KOR')?dt_Data[i].AC_FNM:dt_Data[i].AC_LNM; 
                    ...
                    row_item.getCell(5).value = dt_Data[i].OPEN_DRCR;   
                    row_item.getCell(6).value = dt_Data[i].DR_SUM;  
                    row_item.getCell(7).value = dt_Data[i].CR_SUM;  
                    // row_item.getCell(9).value = dt_Data[i].ACC_DR_SUM;   
                    // row_item.getCell(10).value = dt_Data[i].ACC_CR_SUM;   
                    row_item.getCell(8).value = dt_Data[i].END_DRCR;    
                    // if(_leaf_YN === "ALL" || _leaf_YN === "BAL-PER")
                    // {
                    //     row_item.getCell(12).value = dt_Data[i].LEAF_YN;  
                    // }
                    pos = pos + 1; 
                    ...
                    for (var x = 0; x < dt_Data_sign.length; x++) 
                     {
                        ...
                        {   row_item = worksheet.getRow(pos );
                            ...
                            {
                                row_item.getCell(2).value = dt_Data_sign[x].VAL1 ; 
                               ...
                               // row_item.getCell(4).font.bold = true;
                                row_item.getCell(6).value = dt_Data_sign[x].VAL3;                         
                                row_item.getCell(6).font.bold = true;
                                ...
                                row_item.getCell(6).alignment = { vertical: 'middle', horizontal: 'center' };
                            }
                            ...
                            if (dt_Data_sign[x].CODE =='VIE_SIG')
                            {
                               ...
                               // row_item.getCell(4).font.bold = false;
                                row_item.getCell(6).value = dt_Data_sign[x].VAL3; 
                                row_item.getCell(6).font.bold = false;
                                row_item.getCell(2).alignment = { vertical: 'middle', horizontal: 'center' };
                                row_item.getCell(6).alignment = { vertical: 'middle', horizontal: 'center' };  
                               // row_item.getCell(4).alignment = { vertical: 'middle', horizontal: 'center' };                        
                            }    
