...
class rptCS50031_1 {
    async rpt_CS50031_Billing_1({ request, response, auth }) {
        try {
                /****************************** Get Param *********************************/
            ...
            { 
                    ...
                    var dt_m   = await DBService.callProcCursor("CW_RPT_CS50030_M_NOCACHE", _param , p_language , p_crt_by,item.P_SECOND_DB_YN); 
                    var dt_stock   = await DBService.callProcCursor("CW_RPT_CS50031_STOCK_NOCACHE", _param , p_language , p_crt_by,item.P_SECOND_DB_YN);
                    var dt_service   = await DBService.callProcCursor("CW_RPT_CS50031_SER_NOCACHE", _param , p_language , p_crt_by,item.P_SECOND_DB_YN);
				   ...
				   for (var i = 0; i < stock_merge -1; i++) 
                    {
						...
						if( dt_stock[i].ORD == "0")
						{
							    ...
							    worksheet.getCell("A"+(pos + i)).fill = {  type: 'pattern',
																						pattern:'solid',
																						fgColor:{argb:'b8f6b9'},
																						bgColor:{argb:'b8f6b9'},
																						 bold: true 
