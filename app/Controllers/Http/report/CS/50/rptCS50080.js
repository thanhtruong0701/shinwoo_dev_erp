...
class rptCS50080 {
    async rpt_CS50080({ request, response, auth }) {
        try {
                /****************************** Get Param *********************************/
                ...
                var _param          = [ item.P_COMPANY,item.P_TCO_BUSPLACE_PK,item.P_CONSIGNOR_PK,item.P_DATE_FROM,item.P_DATE_TO]; 
            ...
            { 
                    ...
                    var dt_m   = await DBService.callProcCursor("CW_RPT_CS50060_2_NOCACHE", _param , p_language , p_crt_by); 
					var dt    = await DBService.callProcCursor("CW_RPT_CS50080_1_NOCACHE", _param , p_language , p_crt_by);
                    ...
                    for (var i = 0; i < dt.length; i++) 
                    {
                        ...
                        row_item.getCell(2).value   = dt[i].VOUCHERNO;
						row_item.getCell(4).value   = dt[i].INVOICE_DATE;
                        for (let index = 3; index < Object.keys(dt[0]).length ; index++) 
                        {
                           var data_d= Object.keys(dt[0])[index];
                            row_item.getCell(2+index).value   =  dt[i][data_d];
                        }
                    ...
                    for (let index = 3; index < Object.keys(dt[0]).length ; index++) 
                    {
                        ...
                        row_header.getCell(2+index).value   = data_f;
