...
class rptCS50190 {
    async rpt_CS50190_Billing({ request, response, auth }) {
        try {
            /****************************** Get Param *********************************/
            ...
            var _param = [item.P_COMPANY,item.P_BUSPLACE_PK,item.P_PARTNER_PK,item.P_FROM_DATE,item.P_TO_DATE];
            ...
            /****************************** Begin call store and write excell *********/
                ...
                var dt_m = await DBService.callProcCursor("CW_RPT_CS50190_M_NOCACHE", _param, p_language, p_crt_by, item.SECOND_DB_YN);
                var dt_stock = await DBService.callProcCursor("CW_CS50190_STOCK_NOCACHE", _param, p_language, p_crt_by, item.SECOND_DB_YN);
                var dt_service = await DBService.callProcCursor("CW_RPT_CS50190_SERVICE_NOCACHE", _param, p_language, p_crt_by, item.SECOND_DB_YN); //CW_RPT_CS50040_1_3_NOCACHE 
                ...
                for (var j = 0; j < dt_service.length; j++) {
                    ...
                    row_s.getCell(1).value = dt_service[j].SERVICE_CODE;
                    row_s.getCell(2).value = dt_service[j].SEVICE_NAME_E;
                    ...
                    row_s.getCell(26).value = dt_service[j].TT_AMT;
                ...
                worksheet.getCell("F" + pos_sum).value = dt_m[0].WEIGHT_STORAGE;

                worksheet.mergeCells(pos_sum, 11, pos_sum, 12);
                worksheet.getCell("K" + pos_sum).value = dt_m[0].PLTS_STORAGE;

                worksheet.mergeCells(pos_sum, 17, pos_sum, 20);
                worksheet.getCell("Q" + pos_sum).value = dt_m[0].QTY_HDL;

                worksheet.mergeCells(pos_sum, 22, pos_sum, 24);
                worksheet.getCell("V" + pos_sum).value = dt_m[0].WEIGHT_HDL;
